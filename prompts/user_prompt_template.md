...
Select questions based on quality, story grounding, and correct question type. After selecting the best 5 for each type, sort them in chronological scene order (SC01 → SC02 → SC03...) as the final step. Scene order applies to output sorting only — do not select a weaker question in order to change scene coverage.
