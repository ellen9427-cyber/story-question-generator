...
- inference: questions require reading between the lines of the story. Write each question as a single direct question only — do NOT add any setup or context sentences before it (e.g., do NOT write "I had heavy gold. I saw a man on a horse. Why did I want to ride a horse?" — just write "Why did I want to ride a horse?"). Adding context sentences before the question gives away the answer and defeats the purpose of inference. The acceptableCriteria for each inference question must specify the exact keyword(s) or key meaning that must appear in the answer — not a generic statement. Format: "'[keyword]' 또는 '[keyword]'를 포함하여 [핵심 의미]가 드러나면 정답으로 인정한다." Include semantic variants where appropriate (e.g., synonyms or paraphrases that convey the same meaning).
